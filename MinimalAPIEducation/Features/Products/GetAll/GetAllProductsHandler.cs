...
public class GetAllProductsHandler(AppDbContext context) : IRequestHandler<GetAllProductsQuery, ServiceResult<List<ProductResponse>>>
{
    public async Task<ServiceResult<List<ProductResponse>>> Handle(GetAllProductsQuery request, CancellationToken cancellationToken)
    {
        ...
        return productsFromDb.Count == 0
            ? ServiceResult<List<ProductResponse>>.Error("No products found", HttpStatusCode.NotFound)
            : ServiceResult<List<ProductResponse>>.SuccessAsOk(productsFromDb);
    }
}